...
def test_glcm_4d(size, bins, radius, batches):
    np.random.seed(0)
    ar_batch = np.random.randint(0, bins, [batches, size, size, 3])
...
def test_glcm_4d_cp(size, bins, radius, batches):
    cp.random.seed(0)
    ar_batch = cp.random.randint(0, bins, [batches, size, size, 3])
    glcm = GLCMCross(radius=radius, bin_from=bins, bin_to=bins)
    g_batch = glcm.run(ar_batch)
